...
const DoctorAvailability = () => {
  ...
  const handleEditAvailability = (availability: Availability) => {
    ...
    form.reset({
      ...
      startTime: availability.slots[0]?.startTime || '09:00',
      endTime: availability.slots[0]?.endTime || '10:00',
      hoursAvailable: availability.slots[0]?.hoursAvailable || 1,
      tokenCount: availability.slots[0]?.tokenCount || 10,
      isActive: availability.isActive,
    });
    setShowEditAvailability(true);
  };

  const handleUpdateAvailability = async (formData: any) => {
    try {
      const token = getAuthToken();
      ...
      if (!token) {
        toast({
          title: 'Error',
          description: 'Please log in to update availability',
          variant: 'destructive',
        });
        return;
      }
      ...
      const updatedSlot = {
        ...
        hoursAvailable: parseInt(formData.hoursAvailable),
        tokenCount: parseInt(formData.tokenCount),
        bookedTokens: selectedAvailability.slots[0]?.bookedTokens || []
      };

      const response = await fetch(`${API_URL}/api/doctor-availability/${selectedAvailability.doctorId}/${dayOfWeek}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
                                          ...
                                          <td className="px-6 py-4 whitespace-nowrap text-center">
                                            {(() => {
                                              ...
                                              if (doctorAvailabilities.length === 0) {
                                                return <Badge variant="secondary" className="bg-gray-100 text-gray-600">No Availability</Badge>;
                                              }

                                              // Check if any availability is active
                                              const hasActiveAvailability = doctorAvailabilities.some(av => av.isActive);
                                              if (!hasActiveAvailability) {
                                                return <Badge variant="secondary" className="bg-gray-100 text-gray-600">Inactive</Badge>;
                                              }

                                              // Get the overall availability status
                                              const overallStatus = getAvailabilityStatus(doctorAvailabilities[0]);
                                              return <Badge className={overallStatus.color}>{overallStatus.status}</Badge>;
                                            })()}
                                          </td>
                                          <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <div className="flex items-center justify-center space-x-2">
                                              ...
                                              <span className="text-sm font-medium text-purple-600">
                                                {(() => {
                                                  const doctorAvailabilities = getDoctorAvailability(doctor._id);
                                                  return doctorAvailabilities.reduce((sum, av) => sum + av.slots.length, 0);
                                                })()}
                                              </span>
                                            </div>
                                          </td>
                                          <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <DropdownMenu>
                                              ...
                                              <DropdownMenuContent align="end">
                                                ...
                                                {getDoctorAvailability(doctor._id).length > 0 && (
                                                  <DropdownMenuItem onClick={() => {
                                                    const firstAvailability = getDoctorAvailability(doctor._id)[0];
                                                    handleEditAvailability(firstAvailability);
                                                  }}>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-gray-600 mt-1">Manage doctor availability schedules</p>
                  </div>
                  <Button
                    onClick={() => setShowAddAvailability(true)}
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-4">
                    {doctors.map((doctor) => {
                      const doctorAvailabilities = getDoctorAvailability(doctor._id);
                      const totalSlots = doctorAvailabilities.reduce((sum, av) => sum + av.slots.length, 0);
                      const activeAvailabilities = doctorAvailabilities.filter(av => av.isActive);
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  ...
                                  {doctorAvailabilities.length > 0 && (
                                    <DropdownMenuItem onClick={() => {
                                      const firstAvailability = doctorAvailabilities[0];
                                      handleEditAvailability(firstAvailability);
                                    }}>
                                      <Edit className="w-4 h-4 mr-2" />
                                      Edit Availability
                                        ...
                                        <div className="flex items-center justify-between mb-2">
                                          ...
                                          <div className="flex items-center space-x-2">
                                            ...
                                            <DropdownMenu>
                                              ...
                                              <DropdownMenuContent align="end">
                                                <DropdownMenuItem onClick={() => handleEditAvailability(availability)}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Active availability will be shown to patients for booking
                  </FormDescription>
                </FormItem>
              )}
            />
  ...
  <Dialog open={showEditAvailability} onOpenChange={setShowEditAvailability}>
    <DialogContent className="max-w-2xl">
      <DialogHeader>
        <DialogTitle className="text-2xl font-bold">Edit Doctor Availability</DialogTitle>
        <DialogDescription>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleUpdateAvailability)} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField
              control={form.control}
              name="doctorId"
