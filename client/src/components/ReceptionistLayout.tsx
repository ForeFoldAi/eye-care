...
const receptionistMenuItems = [
  ...
  { icon: IndianRupee, label: "Payments", href: "/receptionist/payments" },
];

function ReceptionistLayoutContent({ user }: { user: AuthUser }) {
  ...
  const { data: hospitalInfo, isLoading: hospitalLoading } = useQuery<HospitalInfo>({
    queryKey: ['receptionist', 'hospital', user.hospitalId],
    queryFn: async () => {
  ...
  const { data: branchInfo, isLoading: branchLoading } = useQuery<BranchInfo>({
    queryKey: ['receptionist', 'branch', user.branchId],
    queryFn: async () => {
  ...
  const { data: departmentInfo, isLoading: departmentLoading } = useQuery<DepartmentInfo>({
    queryKey: ['receptionist', 'department', user.department],
    queryFn: async () => {
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center space-x-1 mt-1">
                  <Users className="h-3 w-3 text-[#8ba1bc]" />
          ...
          <div className="px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="text-center">
                <h1 className="text-lg font-semibold text-gray-900">Receptionist Dashboard</h1>
                <p className="text-xs text-gray-600">Manage patients, appointments and payments</p>
              </div>
  ...
  return (
    <ProtectedRoute requiredRole="receptionist">
      {(currentUser: AuthUser) => (
        <ReceptionistLayoutContent user={currentUser} />
      )}
    </ProtectedRoute>
  );
} 